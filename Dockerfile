...
# update packages, then install without prompts: java development kit, wget, tar
RUN apt-get update && apt-get install -y default-jre wget tar rsync ssh

# create environment variables for spark home directory; add bin/sbin folders to path to allow for spark scripts to run without full reference
ENV SPARK_HOME=/opt/spark
ENV PATH=$SPARK_HOME/bin:$SPARK_HOME/sbin:$PATH

ENV SPARK_VERSION=3.5.3

# create spark directory, download tar, extract tar, remove tar 
